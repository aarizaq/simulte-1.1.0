...
void VoDUDPServer::handleMessageWhenUp(cMessage *msg)
{
    if (msg->isSelfMessage())
    {
        if (!strcmp(msg->getName(), "Timer"))
...
void VoDUDPServer::handleNS2Message(cMessage *msg)
{
    ...
    auto pkt = new Packet("VoDPacket");
    pkt->insertAtFront(frame);
    socket.sendTo(pkt, msgNew->getClientAddr(), msgNew->getClientPort());
...
void VoDUDPServer::handleSVCMessage(cMessage *msg)
{
    ...
    if (svcTrace_[numPkSentApp].index == LONG_MAX)
    {
        ...
        auto fm = new Packet("VoDFinishPacket");
    ...
    else
    {
        ...
        auto pkt = new Packet("VoDPacket");
        auto frame = makeShared<VoDPacket>();//("VoDPacket");
        frame->setFrameSeqNum(seq_num);
        frame->setChunkLength(inet::B(svcTrace_[numPkSentApp].length));
