...
void VoDUDPClient::handleMessage(cMessage* msg)
{
    if (msg->isSelfMessage())
    {
        ...
        receiveStream(check_and_cast<inet::Packet *> (msg));
    }
    else
        delete msg;
}
...
void VoDUDPClient::handleMessageWhenUp(cMessage *msg)
{
    if (msg->isSelfMessage())
    {
        int localPort = par("localPort");
...
void VoDUDPClient::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    ...
    if (dynamicPtrCast<const VoDPacket>(chunk)) {
...
void VoDUDPClient::receiveStream(inet::Packet *pkt)
{
    auto header = pkt->peekAtFront<VoDPacket>();

    int seqNum = header->getFrameSeqNum();
    simtime_t sendingTime = pkt->getCreationTime();
    int frameLength = header->getFrameLength();
    simtime_t delay = simTime() - sendingTime;
    int layer = header->getQid();
