...
void MultihopD2D::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_APPLICATION_LAYER )
    {
        ...
        InterfaceEntry *ie = ift->findInterfaceByName("wlan");
        if (!ie)
        ...
        ltePhy_ = check_and_cast<LtePhyBase*>(getParentModule()->getSubmodule("wlanLteNic")->getSubmodule("phy"));
        LteMacBase* mac = check_and_cast<LteMacBase*>(getParentModule()->getSubmodule("wlanLteNic")->getSubmodule("mac"));
...
void MultihopD2D::handleMessageWhenUp(cMessage *msg)
{
    if (msg->isSelfMessage())
    {
        if (!strcmp(msg->getName(), "selfSender"))
...
void MultihopD2D::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    ...
    auto header  = packet->peekAtFront<MultihopD2DPacket>();
...
void MultihopD2D::sendPacket()
{
    ...
    // create new packet
    auto pkt = new inet::Packet("MultihopD2DPacket");

    auto header  = inet::makeShared<MultihopD2DPacket>();


    header->setMsgid(msgId);
    header->setSrcId(lteNodeId_);
    header->setTimestamp(simTime());
    header->setChunkLength(inet::B(msgSize_));
    header->setTtl(ttl_-1);
    header->setHops(1);                // first hop
    header->setLastHopSenderId(lteNodeId_);
    if (maxBroadcastRadius_ > 0)
    {
        header->setSrcCoord(ltePhy_->getCoord());
        header->setMaxRadius(maxBroadcastRadius_);
    }

    pkt->insertAtFront(header);

    EV << "MultihopD2D::sendPacket - Sending msg "<< header->getMsgid() <<  endl;
...
void MultihopD2D::handleRcvdPacket(cMessage* msg)
{
    ...
    msg->setName("MultihopD2DPacket");
    auto pkt = check_and_cast<inet::Packet*>(msg);
    auto header = pkt->peekAtFront<MultihopD2DPacket>();

    uint32_t msgId = header->getMsgid();
    ...
    else
    {
        ...
        stat_->recordReception(lteNodeId_, msgId, delay, header->getHops());
        ...
        else if (header->getMaxRadius() > 0 && !isWithinBroadcastArea(header->getSrcCoord(), header->getMaxRadius()))
        {
...
void MultihopD2D::relayPacket(cMessage* msg)
{
    ...
    auto pkt = check_and_cast<inet::Packet*>(msg);
    auto header = pkt->removeAtFront<MultihopD2DPacket>();


    // increase the number of hops
    unsigned int hops = header->getHops();
    header->setHops(++hops);
    ...
    EV << "MultihopD2D::relayPacket - Relay msg " << header->getMsgid() << " to address " << destAddress_ << endl;
    stat_->recordSentMessage(header->getMsgid());
    markAsRelayed(header->getMsgid());    // mark the message as relayed
