...
void VoIPSender::handleMessageWhenUp(cMessage *msg)
{
    if (msg->isSelfMessage())
    {
        if (!strcmp(msg->getName(), "selfSender"))
...
{
    auto pkt = new inet::Packet("VoIP");
    auto header = inet::makeShared<VoipPacket>();
    header->setIDtalk(iDtalk_ - 1);
    header->setNframes(nframes_);
    header->setIDframe(iDframe_);
    header->setTimestamp(simTime());
    header->setChunkLength(inet::B(size_));
