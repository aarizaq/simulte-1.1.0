...
cModule* getMacByMacNodeId(MacNodeId nodeId)
{
	...
	// TODO fix for relays
	return (getSimulation()->getModule(getBinder()->getOmnetId(nodeId))->getSubmodule("wlanLteNic")->getSubmodule("mac"));
}
...
void removeAllSimulLteTags(Packet *pkt) {
    auto c2 = pkt->removeTagIfPresent<TftControlInfo>();
    if (c2)
        delete c2;
    auto c3 = pkt->removeTagIfPresent<X2ControlInfo>();
    if (c3)
        delete c3;
