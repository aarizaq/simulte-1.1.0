    ...
    else if (stage == inet::INITSTAGE_PHYSICAL_LAYER)
    {
        ...
        if (dynamicCellAssociation_)
        {
            ...
            for (; it != enbList->end(); ++it)
            {
                ...
                LtePhyBase* cellPhy = check_and_cast<LtePhyBase*>((*it)->eNodeB->getSubmodule("wlanLteNic")->getSubmodule("phy"));
...
void LtePhyUe::triggerHandover()
{
    ...
    IP2lte* enbIp2lte =  check_and_cast<IP2lte*>(getSimulation()->getModule(binder_->getOmnetId(masterId_))->getSubmodule("wlanLteNic")->getSubmodule("ip2lte"));
...
void LtePhyUe::doHandover()
{
    ...
    // update cellInfo
    LteMacEnb* newMacEnb =  check_and_cast<LteMacEnb*>(getSimulation()->getModule(binder_->getOmnetId(candidateMasterId_))->getSubmodule("wlanLteNic")->getSubmodule("mac"));
...
void LtePhyUe::deleteOldBuffers(MacNodeId masterId)
{
    ...
    getSubmodule("wlanLteNic")->getSubmodule("mac"));
    ...
    getSubmodule("wlanLteNic")->getSubmodule("rlc")->getSubmodule("um"));
