...
//        MacNodeId nodeId = it->first.first;
        ...
        // TODO: Change to tag instead header-
...
void LteMacEnbD2D::fromPhy(cPacket *pktAux)
{
    ...
    if (userInfo->getFrameType() == HARQPKT)
    {
        ...
        auto header = pkt->peekAtFront<LteHarqFeedbackMirror>();

        // H-ARQ feedback, send it to mirror buffer of the D2D pair
        MacNodeId d2dSender = header->getD2dSenderId();
        MacNodeId d2dReceiver = header->getD2dReceiverId();
