...
bool LteMacQueue::isEnqueueablePacket(Packet* pkt){
    ...
    auto pdu = dynamicPtrCast<const LteRlcPdu>(chunk);
    //if (pdu == nullptr)
    //    throw cRuntimeError("Header is not of the type LteRlcPdu_Base");
    ...
    if(pdu != NULL){
        if(pdu->getTotalFragments() > 1) {
            bool allFragsWillFit = ((pdu->getTotalFragments()-pdu->getSnoFragment())* pkt->getByteLength() + getByteLength() < queueSize_);
