...
{
    // TODO: Compress header must be review
    return;
...
{
    // TODO: Handle compresses headers
    return;
...
void LtePdcpRrcBase::fromDataPort(cPacket *pktAux)
{
    ...
    // Send message
    send(pkt, (lteInfo->getRlcType() == UM ? umSap_[OUT_GATE] : amSap_[OUT_GATE]));
...
void LtePdcpRrcBase::toDataPort(cPacket *pktAux)
{
    ...
    //FlowControlInfo* lteInfo = check_and_cast<FlowControlInfo*>(pdcpPkt->removeControlInfo());
