...
Packet* LteCompManagerProportional::buildClientRequest()
{
    // build IE
    auto requestIe = makeShared<X2CompProportionalRequestIE>();

    //X2CompProportionalRequestIE* requestIe = new X2CompProportionalRequestIE();
    requestIe->setNumBlocks(provisionedBlocks_);
    auto pkt = new Packet();
    pkt->insertAtFront(requestIe);

    return pkt;
}

//X2CompProportionalReplyIE* LteCompManagerProportional::buildCoordinatorReply(X2NodeId clientId)
Packet* LteCompManagerProportional::buildCoordinatorReply(X2NodeId clientId)
{
    ...
    // build IE
    auto replyIe = makeShared<X2CompProportionalReplyIE>();
    replyIe->setAllowedBlocksMap(allowedBlocks);
    auto pkt = new Packet();
    pkt->insertAtFront(replyIe);

    return pkt;
}

void LteCompManagerProportional::handleClientRequest(Packet* pkt)
{
    ...
    while (compMsg->hasIe())
    {
        auto pktAux = compMsg->popIe();
        auto ie = pktAux->peekAtFront<X2InformationElement>();
        ...
        auto requestIe = dynamicPtrCast<const X2CompProportionalRequestIE>(ie);
        if (requestIe == nullptr)
            throw cRuntimeError("Is not of type X2CompProportionalRequestIE");
...
void LteCompManagerProportional::handleCoordinatorReply(Packet* pkt)
{
    ...
    while (compMsg->hasIe())
    {
        ...
        auto replyIe = dynamicPtrCast<const X2CompProportionalReplyIE>(ie);
        if (replyIe == nullptr)
            throw cRuntimeError("Is not of type X2CompProportionalReplyIE");
        auto allowedBlocksMap = replyIe->getAllowedBlocksMap();
