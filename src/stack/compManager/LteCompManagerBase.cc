...
void LteCompManagerBase::initialize()
{
    ...
    auto pkt = new inet::Packet();
    pkt->addTagIfAbsent<X2ControlInfo>()->setInit(true);
...
void LteCompManagerBase::runClientOperations()
{
    ...
    provisionalSchedule();
    auto requestIe = buildClientRequest();
...
void LteCompManagerBase::runCoordinatorOperations()
{
    ...
    for (; cit != clientList_.end(); ++cit)
    {
        ...
        auto replyIe = buildCoordinatorReply(clientId);
    ...
    {
        // local reply
        auto replyIe = buildCoordinatorReply(nodeId_);
...
void LteCompManagerBase::handleX2Message(Packet* pkt)
{
    ...
    auto compMsg = pkt->peekAtFront<X2CompMsg>();
    ...
    else
    {
        ...
        handleCoordinatorReply(pkt);
    }

    delete pkt;
}

void LteCompManagerBase::sendClientRequest(Packet *inputPacket)
{
    // build control info

    auto requestIe = inputPacket->peekAtFront<X2CompRequestIE>();
...
void LteCompManagerBase::sendCoordinatorReply(X2NodeId clientId, Packet* pktInp)
{
    // build control info

    auto replyIe = pktInp->peekAtFront<X2CompReplyIE>();
    auto pkt = new Packet("X2CompMsg");
    ...
    auto compMsg = makeShared<X2CompMsg>();
    //X2CompMsg* compMsg = new X2CompMsg("X2CompMsg");
    auto pktAux = new Packet("X2CompMsg");
    pktAux->insertAtFront(compMsg);
    compMsg->pushIe(pktAux);
    //compMsg->setControlInfo(ctrlInfo);
